...
impl DaemonSetResourcePanel {
        ...
        match event {
            ResourceUiEvent::ResourcesLoaded { request, result } => match request.kind {
                ...
                ResourceLoadKind::DaemonSets { .. } => {
                    ...
                    match result {
                        Ok(list) => {
                            self.replace_rows(daemon_set_rows_from_list(&list.items));
            ...
            ResourceUiEvent::ResourceActionCompleted { request, result } => {
                ...
                match result {
                    ...
                    Ok(ResourceActionOutcome::Deleted) => {
                        ...
                        {
                            let key = daemon_set_key(namespace.as_deref().unwrap_or(""), &name);
    ...
    fn show_body(
        ...
        match &self.row_status {
            ...
            _ => {
                ...
                let action =
                    show_daemon_set_table(ui, &self.rows, row_indices, &mut self.selected_rows);
    ...
    fn apply_table_action(
        &mut self,
        action: Option<DaemonSetTableAction>,
        cluster_id: &ClusterId,
        requests: &mut Vec<super::ResourceActionRequest>,
    ) {
        match action {
            ...
            Some(DaemonSetTableAction::Restart { key }) => {
                let Some(target) = self.row_by_key(&key).map(DaemonSetRow::delete_target) else {
                    return;
                };
    ...
    fn show_describe_dialog(&mut self, ctx: &egui::Context) {
        ...
        show_resource_describe_window(
            ctx,
            egui::Id::new(("daemon_set-describe-dialog", &dialog.key)),
    ...
    fn replace_rows(&mut self, rows: Vec<DaemonSetRow>) {
        let targets = rows.iter().map(DaemonSetRow::target).collect::<Vec<_>>();
...
fn show_daemon_set_table(
    ...
    let table_width: f32 = SELECT_COLUMN_WIDTH
        + DAEMON_SET_COLUMN_WIDTHS.iter().sum::<f32>()
        + ui.spacing().item_spacing.x * DAEMON_SET_COLUMNS.len() as f32;
    let mut action = None;
                    ...
                    body.rows(row_height, row_indices.len(), |mut table_row| {
                        ...
                        response.context_menu(|ui| {
                            ...
                            if ui
                                ...
                                .clicked()
                            {
                                action = Some(DaemonSetTableAction::Restart {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
                            ...
                            if ui.button(delete_text).clicked() {
                                action = Some(DaemonSetTableAction::Delete {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
...
fn show_daemon_set_describe(ui: &mut egui::Ui, describe: &DaemonSetDescribe) {
    ...
    describe_group(
        ...
        "Conditions",
        |ui| {
            describe_conditions(ui, "daemon_set-describe-conditions", &describe.conditions);
        },
    );
    ...
    describe_group(ui, egui_phosphor::regular::CODE, "Raw manifest", |ui| {
        describe_raw_manifest(
            ui,
            "daemon_set-describe-raw-manifest-content",
            &describe.raw_yaml,
        );
    });
}
...
fn restart_patch() -> serde_json::Value {
    restart_patch_with_timestamp(&utc_now_rfc3339_seconds())
}
...
mod tests {
    ...
    fn restart_patch_sets_restarted_at_annotation() {
        let patch = restart_patch_with_timestamp("2026-05-25T10:00:00Z");
    ...
    #[test]
    fn daemon_set_delete_target_uses_namespace_and_name() {
        let row = DaemonSetRow::from_summary(&daemon_set_summary());
