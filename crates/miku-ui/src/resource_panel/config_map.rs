...
impl ConfigMapResourcePanel {
            ...
            ResourceUiEvent::ResourceActionCompleted { request, result } => {
                ...
                match result {
                    Ok(ResourceActionOutcome::Applied(summary)) => {
                        self.upsert_row(ConfigMapRow::from_summary(&summary));
    ...
    fn apply_table_action(&mut self, action: Option<ConfigMapTableAction>) {
        match action {
            ...
            Some(ConfigMapTableAction::Edit { key }) => {
            ...
            Some(ConfigMapTableAction::Delete { key }) => {
    ...
    fn upsert_row(&mut self, row: ConfigMapRow) {
                    ...
                    body.rows(row_height, row_indices.len(), |mut table_row| {
                        ...
                        response.context_menu(|ui| {
                            ...
                            if ui
                                ...
                                .clicked()
                            {
                                action = Some(ConfigMapTableAction::Edit {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
                            ...
                            if ui.button(delete_text).clicked() {
                                action = Some(ConfigMapTableAction::Delete {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
...
mod tests {
    ...
    fn edit_action_opens_edit_dialog_with_editable_yaml() {
        let mut panel = ConfigMapResourcePanel::default();
        let row = ConfigMapRow::from_summary(&with_server_fields(config_map_summary()));
        let key = row.key.clone();
        panel.rows = vec![row];

        panel.apply_table_action(Some(ConfigMapTableAction::Edit { key }));

        let dialog = panel.edit_dialog.as_ref().unwrap();
        assert_eq!(dialog.target.namespace.as_deref(), Some("default"));
        assert_eq!(dialog.target.name, "app-config");
    ...
    fn delete_action_opens_delete_dialog() {
        ...
        panel.apply_table_action(Some(ConfigMapTableAction::Delete { key }));
    ...
    fn apply_completion_closes_edit_dialog_and_upserts_sorted_row() {
        let mut panel = ConfigMapResourcePanel::default();
        let row = ConfigMapRow::from_summary(&config_map_summary());
        panel.rows = config_map_rows_from_list(&[
            config_map_summary_with_name("zeta", "worker"),
            config_map_summary_with_name("default", "api-b"),
        ]);
        panel.edit_dialog = Some(GenericEditDialog {
            target: row.target(),
            yaml: "kind: ConfigMap".to_owned(),
            parse_error: None,
        });
    ...
    fn action_errors_keep_current_dialogs() {
        let row = ConfigMapRow::from_summary(&config_map_summary());
        let mut edit_panel = ConfigMapResourcePanel {
        ...
        let mut delete_panel = ConfigMapResourcePanel {
