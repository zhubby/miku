...
impl LimitRangeResourcePanel {
        ...
        match event {
            ResourceUiEvent::ResourcesLoaded { request, result } => match request.kind {
                ...
                ResourceLoadKind::LimitRanges { .. } => {
                    ...
                    match result {
                        Ok(list) => {
                            self.replace_rows(limit_range_rows_from_list(&list.items));
            ...
            ResourceUiEvent::ResourceActionCompleted { request, result } => {
                ...
                match result {
                    Ok(ResourceActionOutcome::Applied(summary)) => {
                        self.upsert_row(LimitRangeRow::from_summary(&summary));
                    ...
                    Ok(ResourceActionOutcome::Deleted) => {
                        if let ResourceActionKind::DeleteResource {
                            ...
                            && resource == limit_range_metadata().resource
                        {
    ...
    fn show_body(&mut self, ui: &mut egui::Ui) {
        match &self.row_status {
            ...
            _ => {
                ...
                let action =
                    show_limit_range_table(ui, &self.rows, row_indices, &mut self.selected_rows);
    ...
    fn apply_table_action(&mut self, action: Option<LimitRangeTableAction>) {
        match action {
            ...
            Some(LimitRangeTableAction::Edit { key }) => {
            ...
            Some(LimitRangeTableAction::Delete { key }) => {
    ...
    fn replace_rows(&mut self, rows: Vec<LimitRangeRow>) {
        let targets = rows.iter().map(LimitRangeRow::target).collect::<Vec<_>>();
        let visible_keys = visible_keys(&targets);
        self.selected_rows.retain(|key| visible_keys.contains(key));
        self.rows = rows;
    }

    fn upsert_row(&mut self, row: LimitRangeRow) {
                    ...
                    body.rows(row_height, row_indices.len(), |mut table_row| {
                        ...
                        response.context_menu(|ui| {
                            ...
                            if ui
                                ...
                                .clicked()
                            {
                                action = Some(LimitRangeTableAction::Edit {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
                            ...
                            if ui.button(delete_text).clicked() {
                                action = Some(LimitRangeTableAction::Delete {
                                    key: row.key.clone(),
                                });
                                ui.close();
                            }
...
mod tests {
    ...
    fn edit_action_opens_edit_dialog_with_editable_yaml() {
        let mut panel = LimitRangeResourcePanel::default();
        let row = LimitRangeRow::from_summary(&with_server_fields(limit_range_summary()));
        let key = row.key.clone();
        panel.rows = vec![row];

        panel.apply_table_action(Some(LimitRangeTableAction::Edit { key }));

        let dialog = panel.edit_dialog.as_ref().unwrap();
        assert_eq!(dialog.target.namespace.as_deref(), Some("default"));
        assert_eq!(dialog.target.name, "defaults");
    ...
    fn delete_action_opens_delete_dialog() {
        ...
        panel.apply_table_action(Some(LimitRangeTableAction::Delete { key }));
    ...
    fn apply_completion_closes_edit_dialog_and_upserts_sorted_row() {
        let mut panel = LimitRangeResourcePanel::default();
        let row = LimitRangeRow::from_summary(&limit_range_summary());
        panel.rows = limit_range_rows_from_list(&[
            limit_range_summary_with_name("zeta", "worker"),
            limit_range_summary_with_name("default", "api-b"),
        ]);
        panel.edit_dialog = Some(GenericEditDialog {
            target: row.target(),
            yaml: "kind: LimitRange".to_owned(),
            parse_error: None,
        });
    ...
    fn action_errors_keep_current_dialogs() {
        let row = LimitRangeRow::from_summary(&limit_range_summary());
        let mut edit_panel = LimitRangeResourcePanel {
        ...
        let mut delete_panel = LimitRangeResourcePanel {
